...
if __name__ == "__main__":
    ...
    data2 = PlaceholdersUtil.generate_placeholder_dataset(AbruptDriftPlaceholders.PLACEHOLDER_VALUES_P2)



    # 8. Trie Preperation - Abrupt Drift.
    # trie 1 filled with concept 1
    now = datetime.now().timestamp()
    trie1 = NGramPrefixTree.Trie()
    trie1 = NGramPrefixTree.load_dataset_to_ngram_prefix_tree_text2(trie1, data1, timestamp=now - 30 * 24 * 3600 , tokenizer=tokenizer)
    # trie1 = NGramPrefixTree.load_dataset_to_ngram_prefix_tree_text(trie1, combined_data_p1, tokenizer)
    # trie1 = NGramPrefixTree.load_dataset_to_ngram_prefix_tree_text(trie1, test_data_p1, timestamp=now - 30 * 24 * 3600 , tokenizer=tokenizer)


    # # trie 1 filled with concept 1
    trie2 = NGramPrefixTree.Trie()
    trie2 = NGramPrefixTree.load_dataset_to_ngram_prefix_tree_text2(trie2, data2, timestamp=now, tokenizer=tokenizer)
    # trie2 = NGramPrefixTree.load_dataset_to_ngram_prefix_tree_text(trie2, combined_data_p2, tokenizer)
    # trie2 = NGramPrefixTree.load_dataset_to_ngram_prefix_tree_text(trie2, test_data_p2, timestamp=now, tokenizer=tokenizer)
    ...
    OddMethod1.response_strategy_based_extractor_and_saver_abrupt1(model, tokenizer, test_data_p1p2
                                                           , device, BASE_CONFIG, trie1, trie2
                                                           , SCORING_CONFIG)
