...
from DriftManager import PlaceholdersUtil, IncrementalDriftPlaceholders
import os
...
if __name__ == "__main__":
    ...
    # Split test data into 5 incremental portions (p1 to p5)
    ...
    test_data_p5 = BitextTelecoDS.preprocess_bitext_ds2(
        test_data.select(range(4 * portion, n_test)),
        placeholder="p5",
        drift_type="incremental"
    )
    ...
    # Merge them back
    test_data_p1p5 = concatenate_datasets(
        [test_data_p1, test_data_p2, test_data_p3, test_data_p4, test_data_p5])  # preserve their sequence.
    ...
    test_dataset_p1p5 = InstructionDataset(test_data_p1p5, tokenizer)
    ...
    test_loader_p1p5 = DataLoading.make_loader(test_dataset_p1p5, batch_size=8, shuffle=False, collate_fn=raw_collate)
    ...
    LossCalculator.print_model_loss_for_all_datasets(model, train_loader_p1, test_loader_p1p5, val_loader_p1, device)
    print("--- ---- ---- ---- ---- ---- ")



    data1 = PlaceholdersUtil.generate_placeholder_dataset(IncrementalDriftPlaceholders.PLACEHOLDER_VALUES_P1)
    data2 = PlaceholdersUtil.generate_placeholder_dataset(IncrementalDriftPlaceholders.PLACEHOLDER_VALUES_P2)
    data3 = PlaceholdersUtil.generate_placeholder_dataset(IncrementalDriftPlaceholders.PLACEHOLDER_VALUES_P3)
    data4 = PlaceholdersUtil.generate_placeholder_dataset(IncrementalDriftPlaceholders.PLACEHOLDER_VALUES_P4)
    data5 = PlaceholdersUtil.generate_placeholder_dataset(IncrementalDriftPlaceholders.PLACEHOLDER_VALUES_P5)
    ...
    trie4 = NGramPrefixTree.load_dataset_to_ngram_prefix_tree_text2(trie4, data4, timestamp=now - 10 * 24 * 3600,
                                                                    tokenizer=tokenizer)

    # # trie 1 filled with concept 1
    trie5 = NGramPrefixTree.Trie()
    trie5 = NGramPrefixTree.load_dataset_to_ngram_prefix_tree_text2(trie5, data5, timestamp=now,
                                                                    tokenizer=tokenizer)


    # 10. Text Generators
    # 10.1 Greedy
    Greedy.response_strategy_based_extractor_and_saver(model, tokenizer, test_data_p1p5, device, BASE_CONFIG)
    #
    # # 10.2 Topk-TempScaling
    TopKTempretureScaling.response_strategy_based_extractor_and_saver(model, tokenizer, test_data_p1p5, device, BASE_CONFIG)
    ...
    OddMethod1.response_strategy_based_extractor_and_saver_incremental(model, tokenizer, test_data_p1p5
                                                           , device, BASE_CONFIG, trie1, trie2, trie3, trie4, trie5
                                                           , SCORING_CONFIG)
