# Updated D(clip), with Updated C (C omputation is same - but we changed how we reward using it)
...
def response_strategy_based_extractor_and_saver_abrupt3(model, tokenizer, test_data, device, BASE_CONFIG, trie1, trie2, SCORING_CONFIG):
    ...
    for i, entry in tqdm(enumerate(test_data), total=len(test_data)):
        ...
        print("model response after cleaning=>", response_text)
    ...
    Util.store_in_json_file(test_data, Utils.Constants.RESPONSE_SPINE_FILE_NAME3)



def response_strategy_based_extractor_and_saver_abrupt_all_3(model, tokenizer, test_data, device, BASE_CONFIG, trie1, trie2, SCORING_CONFIG):
    n_test = len(test_data)
    half = n_test // 2
    ...
    for i, entry in tqdm(enumerate(test_data), total=len(test_data)):
        ...
        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        print("response=>", test_data[i]["response"])
    Util.store_in_json_file(test_data, Utils.Constants.RESPONSE_SPINE_FILE_NAME_ALL_3)
...
def test_case(p_lm, p_trie, last_matched_count):
    ...
    c_lm_adj = c_lm * (1 - D**2)
    c_trie_adj = c_trie + (1 - c_trie) * 0.25 * C
    gamma = c_lm_adj / (c_lm_adj + c_trie_adj + 1e-12)
...
if __name__ == "__main__":
    print("\n--- Realistic ODD Behavior Tests ---")

    # 1. Highly confident LLM (should win)
    test_case(torch.tensor([0.97, 0.02, 0.01]), torch.tensor([0.4, 0.3, 0.3]), last_matched_count=0)

    # 2. Strong Trie signal (should win)
    test_case(torch.tensor([0.34, 0.33, 0.33]), torch.tensor([0.85, 0.10, 0.05]), last_matched_count=3)

    # 3. Moderate disagreement (balanced)
    test_case(torch.tensor([0.7, 0.2, 0.1]), torch.tensor([0.2, 0.7, 0.1]), last_matched_count=1)

    # 4. Increasing continuity effect
    for m in [0, 2, 5]:
        test_case(torch.tensor([0.6, 0.25, 0.15]), torch.tensor([0.7, 0.2, 0.1]), last_matched_count=m)

    # 5. Confident but conflicting LLM (margin clip test)
    test_case(torch.tensor([0.95, 0.03, 0.02]), torch.tensor([0.02, 0.95, 0.03]), last_matched_count=0)