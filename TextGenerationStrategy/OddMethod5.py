# #### **(6) Adaptive D Weighting by C**
#
# * When continuity is high, disagreement should matter less.
#   ( D_{\text{eff}} = D \times (1 - 0.5C) )
# * Then ( c'*{\text{LM}} = c*{\text{LM}} (1 - D_{\text{eff}}^2) ).
# * Purpose: smoother handover from LLM to Trie during consistent runs.
...
def response_strategy_based_extractor_and_saver_abrupt5(model, tokenizer, test_data, device, BASE_CONFIG, trie1, trie2, SCORING_CONFIG):
    ...
    Util.store_in_json_file(test_data, Utils.Constants.RESPONSE_SPINE_FILE_NAME5)



def response_strategy_based_extractor_and_saver_abrupt_all_5(model, tokenizer, test_data, device, BASE_CONFIG, trie1, trie2, SCORING_CONFIG):
    n_test = len(test_data)
    half = n_test // 2
    ...
    for i, entry in tqdm(enumerate(test_data), total=len(test_data)):
        ...
        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        print("response=>", test_data[i]["response"])
    Util.store_in_json_file(test_data, Utils.Constants.RESPONSE_SPINE_FILE_NAME_ALL_5)

# === Adaptive D Weighting by C (Variant #6) Main Test ===
def test_case(p_lm, p_trie, last_matched_count):
    ...
    # Adaptive disagreement weighting
    D_eff = D * (1 - 0.5 * C)
    c_lm_adj = c_lm * (1 - D_eff**2)
