# D is changed to clip , No C at all
...
def response_strategy_based_extractor_and_saver_abrupt2(model, tokenizer, test_data, device, BASE_CONFIG, trie1, trie2, SCORING_CONFIG):
    ...
    Util.store_in_json_file(test_data, Utils.Constants.RESPONSE_SPINE_FILE_NAME2)



def response_strategy_based_extractor_and_saver_abrupt_all_2(model, tokenizer, test_data, device, BASE_CONFIG, trie1, trie2, SCORING_CONFIG):
    n_test = len(test_data)
    half = n_test // 2
    ...
    for i, entry in tqdm(enumerate(test_data), total=len(test_data)):
        ...
        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        print("response=>", test_data[i]["response"])
    Util.store_in_json_file(test_data, Utils.Constants.RESPONSE_SPINE_FILE_NAME_ALL_2)

if __name__ == "__main__":
    print("\n--- Test Variant #2: D clipped, No C ---\n")

    def test_case(p_lm, p_trie):
        # Compute disagreement with clipping logic
        D = compute_disagreement_D(p_lm, p_trie, k=8, conf_lm=p_lm)
        c_lm = compute_llm_confidence(p_lm)
        c_trie = float(p_trie.max().item())

        # Apply penalization using clipped D
        c_lm_adj = c_lm * (1 - D**2)
    ...
    test_case(torch.tensor([0.97, 0.02, 0.01]), torch.tensor([0.4, 0.3, 0.3]))   # confident LLM
    test_case(torch.tensor([0.6, 0.3, 0.1]), torch.tensor([0.3, 0.6, 0.1]))     # Trie stronger
    test_case(torch.tensor([0.5, 0.3, 0.2]), torch.tensor([0.7, 0.2, 0.1]))     # aligned top token
    test_case(torch.tensor([0.9, 0.05, 0.05]), torch.tensor([0.05, 0.9, 0.05])) # strong conflict
