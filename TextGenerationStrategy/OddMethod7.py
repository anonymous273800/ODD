# #### **(8) Continuity Momentum**
#
# * Make C cumulative with mild decay:
#   `C_t = 0.8*C_{t-1} + 0.2*(1 - exp(-matches/3))`
# * Purpose: maintain continuity memory across boundaries (helps with sustained consistency).
...
def generate_greedy7(
    ...
    C_mem = [0.0 for _ in range(idx.size(0))]  # continuity momentum memory
    ...
    for i in range(max_new_tokens):
        ...
        for b in range(batch_size):
            ...
            C_inst = compute_continuity_C(last_matched_count[b])

            # --- Apply continuity momentum ---
            C_mem[b] = 0.8 * C_mem[b] + 0.2 * C_inst
            C = C_mem[b]

            # --- Get top token for LLM ---
            ...
            print("Trie Confidence (After Rewarding)", c_trie_adj)

            # --- Compute gamma ---
            gamma = c_lm_adj / (c_lm_adj + c_trie_adj + 1e-12)
            ...
            if top_mix_token == eos_id:
                end_flag = True

            print(f"c_lm_adj: {c_lm_adj:.3f} | gamma: {gamma:.3f} | D={D:.3f}, C_inst={C_inst:.3f}, C_mem={C:.3f}")
            print("SUMMARY LLM:", top_llm_token, Util.token_ids_to_text(torch.tensor([[top_llm_token]], device=logits.device), tokenizer), f"({top_llm_prob:.4f})")
            print("SUMMARY TRIE:", top_trie_token, Util.token_ids_to_text(torch.tensor([[top_trie_token]], device=logits.device), tokenizer), f"({top_trie_score:.4f})")
            print("SUMMARY MIX:", top_mix_token, Util.token_ids_to_text(torch.tensor([[top_mix_token]], device=logits.device), tokenizer), f"({top_mix_prob:.4f})")
            print("----- ----- ---- --- ")
...
def response_strategy_based_extractor_and_saver_abrupt7(model, tokenizer, test_data, device, BASE_CONFIG, trie1, trie2, SCORING_CONFIG):
    ...
    Util.store_in_json_file(test_data, Utils.Constants.RESPONSE_SPINE_FILE_NAME7)



def response_strategy_based_extractor_and_saver_abrupt_all_7(model, tokenizer, test_data, device, BASE_CONFIG, trie1, trie2, SCORING_CONFIG):
    n_test = len(test_data)
    half = n_test // 2
    ...
    for i, entry in tqdm(enumerate(test_data), total=len(test_data)):
        ...
        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        print("response=>", test_data[i]["response"])
    Util.store_in_json_file(test_data, Utils.Constants.RESPONSE_SPINE_FILE_NAME_ALL_7)


# === Continuity Momentum (Variant #8) Main Test ===
def test_case(p_lm, p_trie, last_matched_count, C_prev):
    c_lm = compute_llm_confidence(p_lm)
    c_trie = float(p_trie.max().item())
    D = compute_disagreement_D(p_lm, p_trie, k=8, conf_lm=c_lm)
    C_inst = compute_continuity_C(last_matched_count)

    # Apply continuity momentum update
    C_mem = 0.8 * C_prev + 0.2 * C_inst

    # Confidence adjustments
    c_lm_adj = c_lm * (1 - D ** 2)
    c_trie_adj = c_trie + (1 - c_trie) * 0.25 * C_mem
    gamma = c_lm_adj / (c_lm_adj + c_trie_adj + 1e-12)
    ...
    print(f"c_lm={c_lm:.3f}, c_trie={c_trie:.3f}, D={D:.3f}, C_inst={C_inst:.3f}, C_mem={C_mem:.3f}")
    print(f"c_lm_adj={c_lm_adj:.3f}, c_trie_adj={c_trie_adj:.3f}, gamma={gamma:.3f}")
    print(f"Top LLM token={top_lm}, Top Trie token={top_trie} → Winner: {winner}")
    return C_mem


if __name__ == "__main__":
    print("\n--- Variant #8: Continuity Momentum Tests ---")

    C_prev = 0.0  # initialize continuity memory

    # 1️⃣ Confident LLM start — should win
    C_prev = test_case(torch.tensor([0.97, 0.02, 0.01]),
                       torch.tensor([0.4, 0.3, 0.3]),
                       last_matched_count=0, C_prev=C_prev)

    # 2️⃣ Strong Trie, low LLM confidence — Trie dominates, memory grows
    C_prev = test_case(torch.tensor([0.34, 0.33, 0.33]),
                       torch.tensor([0.85, 0.10, 0.05]),
                       last_matched_count=3, C_prev=C_prev)

    # 3️⃣ Balanced conflict — both contribute
    C_prev = test_case(torch.tensor([0.7, 0.2, 0.1]),
                       torch.tensor([0.2, 0.7, 0.1]),
                       last_matched_count=1, C_prev=C_prev)

    # 4️⃣ Sequence of growing matches — C_mem accumulates momentum
    for m in [0, 2, 5]:
        C_prev = test_case(torch.tensor([0.6, 0.25, 0.15]),
                           torch.tensor([0.7, 0.2, 0.1]),
                           last_matched_count=m, C_prev=C_prev)

    # 5️⃣ Cross-confident case — LLM and Trie opposite, low continuity
    C_prev = test_case(torch.tensor([0.95, 0.03, 0.02]),
