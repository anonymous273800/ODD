# #### **(7) Symmetric Reward/Penalty Scaling**
#
# * Apply both symmetrically:
#   ( c'*{\text{LM}} = c*{\text{LM}} (1 - 0.5D^2) )
#   ( c'*{\text{Trie}} = c*{\text{Trie}} + (1 - c_{\text{Trie}}) \times 0.5C )
# * Purpose: reduce aggressiveness on both sides to see if smoother transitions yield better balance.
...
def generate_greedy6(
    ...
    for i in range(max_new_tokens):
        ...
        for b in range(batch_size):
            ...
            # --- Adjust confidences (symmetric) ---
            c_lm_adj = c_lm_base * (1 - 0.5 * D ** 2)
            c_trie_adj = c_trie_base + (1 - c_trie_base) * 0.5 * C
...
def response_strategy_based_extractor_and_saver_abrupt6(model, tokenizer, test_data, device, BASE_CONFIG, trie1, trie2, SCORING_CONFIG):
    ...
    Util.store_in_json_file(test_data, Utils.Constants.RESPONSE_SPINE_FILE_NAME6)




def response_strategy_based_extractor_and_saver_abrupt_all_6(model, tokenizer, test_data, device, BASE_CONFIG, trie1, trie2, SCORING_CONFIG):
    n_test = len(test_data)
    half = n_test // 2
    ...
    for i, entry in tqdm(enumerate(test_data), total=len(test_data)):
        ...
        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        print("response=>", test_data[i]["response"])
    Util.store_in_json_file(test_data, Utils.Constants.RESPONSE_SPINE_FILE_NAME_ALL_6)

# === Symmetric Reward/Penalty Scaling (Variant #7) Main Test ===
def test_case(p_lm, p_trie, last_matched_count):
    ...
    # Symmetric reward/penalty scaling
    c_lm_adj = c_lm * (1 - 0.5 * D ** 2)
    c_trie_adj = c_trie + (1 - c_trie) * 0.5 * C
    gamma = c_lm_adj / (c_lm_adj + c_trie_adj + 1e-12)
